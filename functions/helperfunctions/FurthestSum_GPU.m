function i=FurthestSum_GPU(K,noc,i,exclude)
...
if J>I % Fast implementation for large scale number of observations. Can be improved by reusing calculations
    ...
    for k=1:noc+10            
        if k>noc-1 % Remove initial seed        
           Kq=Kt(:,i(1))'*Kt;        
           sum_dist=sum_dist-sqrt(bsxfun(@max,Kt2-2*Kq+Kt2(i(1)),0));                 
           index(i(1))=i(1);
           i(1)=[];                       
        end   
        t=find(index);
        Kq=Kt(:,ind_t)'*Kt;        
        sum_dist=sum_dist+sqrt(bsxfun(@max,Kt2-2*Kq+Kt2(ind_t),0));                                    
    ...
    if I~=J || sum(sum(K-K'))~=0 % Generate kernel if K not a kernel matrix
       Kt=K;
       K=Kt'*Kt;        
       K=sqrt(bsxfun(@max,bsxfun(@plus,bsxfun(@minus,diag(K)',2*K),diag(K)),0));            
    end
    Kt2=diag(K)';
    for k=1:noc+10                            
        if k>noc-1
            sum_dist=sum_dist-sqrt(bsxfun(@max,Kt2-2*K(i(1),:)+Kt2(i(1)),0));
            index(i(1))=i(1);
            i(1)=[];                    
        end           
        t=find(index);
        sum_dist=sum_dist+sqrt(bsxfun(@max,Kt2-2*K(ind_t,:)+Kt2(ind_t),0));
        [val,ind]=max(sum_dist(t));        
        ind_t=t(ind(1));
        i=[i ind_t];        
        index(t(ind(1)))=0;
